...
class User extends Base
{
    ...
    {
        $sql = "UPDATE `mac_user` SET `user_play_nums` = '3', `user_cahce_nums` = '3'";
        Db::query($sql);
        return ['code' => 1, 'msg' => '更新成功'];
    }

    public function register($param,$type = 0)
    {
        $config = config('maccms');

        $data = [];
        $type == 2 ? $param['country_code'] = $param['country_code'] : $param['country_code'] = '';
        $data['user_name'] = htmlspecialchars(urldecode(trim($param['country_code'].$param['user_name'])));
        ...
        $data['user_reward'] = $param['user_reward'];
        $data['id'] = $param['id'];

        $data['jid'] = $param['jid'];
        $data['os'] = intval($param['os']);
        $data['osname'] = $param['osname'];
        ...
        //邀请码
        if ($data['user_reward']) {
            $row = $this->where('user_reward', $data['user_reward'])->find();
            if (!empty($row)) {
                $data['user_pid'] = intval($row['user_id']);
            }else{
              return ['code' => 1011, 'msg' => '无效邀请码'];
            }
        }
        ...
        $fields['user_reward'] = $this->random();
        $fields['user_play_update'] = date("Y-m-d",time());

        if(!empty($fields['user_reward'])){
            $where7=[];
            $where7['user_reward'] = ['eq',$fields['user_reward']];
            $data7 = model('User')->infoData($where7);
            $fields['user_reward'] = $this->random();
        }
        $fields['user_pid'] = $data['user_pid'] > 0 ? $data['user_pid'] : 0;
        $fields['id'] = $data['id'];
        $fields['jid'] = $data['jid'];
        $fields['os'] = intval($data['os']);
        $fields['osname'] = $data['osname'];
        ...
        if(!empty($fields['id'])){
          ...
          $fields['user_play_num']    = $data['code'] == 1 ? $data['info']['play_num'] : 3;
          $fields['user_play_nums']   = $data['code'] == 1 ? $data['info']['play_nums'] : 3;
          $fields['user_cahce_num']   = 3;
          $fields['user_cahce_nums']  = 3;
        }else{
          $fields['user_play_num']    = 3;
          $fields['user_play_nums']   = 3;
          $fields['user_cahce_num']   = 3;
          $fields['user_cahce_nums']  = 3;
        }

        //邀请观影+2
        if($fields['user_pid'] > 0){
          $where8=[];
          $where8['user_id'] = ['eq',$fields['user_pid']];
          $data8 = model('User')->infoData($where8);

          $where8=[];
          $where8['user_id'] = ['eq',$fields['user_pid']];
          model('User')->fieldData($where8,'user_play_num',$data8['info']['user_play_num']+2);
          model('User')->fieldData($where8,'user_play_nums',$data8['info']['user_play_nums']+2);
          model('User')->fieldData($where8,'user_cahce_num',$data8['info']['user_cahce_num']+1);
          model('User')->fieldData($where8,'user_cahce_nums',$data8['info']['user_cahce_nums']+1);
        }
    ...
    public function info($param, $info = array())
    {
        ...
        //api
        if (count($info)>0) {
            $GLOBALS['user']['user_id'] = $info['user_id'];
            $GLOBALS['user']['user_name'] = $info['user_name'];
            $GLOBALS['user']['user_pwd'] = $info['user_pwd'];
        }
    ...
    public function editinfo($param, $info = array())
    {
        $data = [];
        $data['user_nick_name'] = htmlspecialchars(urldecode(trim($param['nickname'])));

        $where = [];
        $where['user_id'] = ['eq', $info['user_id']];
        $res = $this->where($where)->update($data);
        return ['code' => 1, 'msg' => '保存成功'];
        ;
    }

    public function login($param, $type = 0)
    {
        $type == 2 ? $param['country_code'] = $param['country_code'] : $param['country_code'] = '';
        $data = [];
        $data['user_name'] = htmlspecialchars(urldecode(trim($param['country_code'].$param['user_name'])));
        ...
        $data['code'] = htmlspecialchars(urldecode(trim($param['code'])));
        $data['id'] = htmlspecialchars(urldecode(trim($param['id'])));

        $data['jid'] = htmlspecialchars(urldecode(trim($param['jid'])));
        $data['os'] = htmlspecialchars(urldecode(trim($param['os'])));
        $data['osname'] = htmlspecialchars(urldecode(trim($param['osname'])));
        ...
        !empty($data['jid']) && $update['jid'] = $data['jid'];
        !empty($data['os']) && $update['os'] = intval($data['os']);
        !empty($data['osname']) && $update['osname'] = $data['osname'];

        $where6 = [];
        $where6['user_pid'] = ['eq',$row['user_id']];
        $where6['admin_play'] = ['eq',0];
        $reward_total = model('User')->countData($where6);
        if($reward_total > 0){
          if($reward_total > 0 && $reward_total < 3){
            $update['user_play_num'] = 3 + 2;
            $update['user_cache_num'] = 3 + 1;
          }elseif ($reward_total > 2 && $reward_total < 10) {
            $update['user_play_num'] = 3 + 5;
            $update['user_cache_num'] = 3 + 3;
          }elseif ($reward_total > 9 && $reward_total < 30) {
            $update['user_play_num'] = 3 + 30;
            $update['user_cache_num'] = 3 + 10;
          }elseif ($reward_total > 29 && $reward_total < 50) {
            $update['user_play_num'] = 3 + 50;
            $update['user_cache_num'] = 3 + 30;
          }elseif ($reward_total > 49) {
            $update['user_play_num'] = 3 + 9999996;
            $update['user_cache_num'] = 3 + 30;
          }
        }
        ...
        $info['user_nick_name']   = $row['user_nick_name'];
        $info['user_portrait']    = Env::get('upload.upload_domain').$row['user_portrait'];
        $info['user_pwd']         = $row['user_pwd'];
        $info['user_reward']      = $row['user_reward'];
        $info['user_money']       = $row['user_money'];

        $domain = Env::get('domain.url');
        $domain = explode(",",$domain);
        $info['reward_url']      = $domain[rand(0,count($domain)-1)].'/?reward='.$row['user_reward'];

        return ['code' => 1, 'msg' => '登录成功', 'userinfo' => $info];
    }

    //更新 token
    public function settoken($user_id, $token, $user_token_time)
    {
        $where['user_id'] = $user_id;
        $update['user_token'] = $token;
        $update['user_token_time'] = $user_token_time;
        $res = $this->where($where)->update($update);
        if ($res === false) {
            return ['code' => 1004, 'msg' => '更新登录信息失败'];
        }
